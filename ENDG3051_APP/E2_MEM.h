...
class E2_MEM
{
  ...
  uint8_t max_rec_size_B; // max record size in bytes / max 64B

public:
  E2_MEM(uint8_t _port_addr = 0, uint8_t _mrsB = 16); // init slave eeprom's address //doesnt have unique value check (may crush on the bus);
  ~E2_MEM(){};
