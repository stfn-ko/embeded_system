...
LogTask::LogTask(int _timestamp) : tm(_timestamp)
{
  ...
  unsigned char iicregs[2];
  iicregs[0]=0x00; // RTCSEC register address
  iicregs[1]=0x80; // set ST bit to start oscillator.
...
int LogTask::SetDate(int dow, int day, int month, int year, int hrs, int mins, int secs, boolean is24hr, boolean ampm)
{
  ...
  do {
    ...
    if(is24hr)
      date.regs.hour=((hrs%10)|((hrs/10)<<4))|0x00; // 24 format check
    else if(ampm)
    ...
    date.regs.date=((day%10)|((day/10)<<4))|0x00; //check
    date.regs.month=((month%10)|((month/10)<<4))|0x00; //add leap year thing
...
void LogTask::LogToSerial(char * message)
{
  ...
  if(is12hr){
    hours=(date.hour&0x0f)+(10*((date.hour>>4)&0x07-6)); 
    amIndicator = const_cast<char*>("pm");  // edit to_check
    if(ampm) amIndicator = const_cast<char*>("am");  // edit to_check
  } 
